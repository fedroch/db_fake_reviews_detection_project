...
def print_base_info(df):
    ...
    print('первая строка:')
    print(df.head(1))
    print('кол-во нулевых значений в каждом столбце:')
    print(df.isnull().sum())
...
# raw_data = pd.read_csv(script_dir / 'data/raw/fake reviews dataset.csv') # маленький датасет
raw_data = pd.read_csv(script_dir / 'data/raw/pseudo_labeled_amazon_reviews.csv') # жирненький датасет
...
# clear_data['text'] = clear_data['text'].apply(lemmatize_text_with_POS_nltk) # лемматизация текста с учетом контекста через nltk (пока пока проц, но скор самы лучший)
